...
var preview = "";
(function($){
	$.fn.Apollo = function(options){

		var $_Apollo = this;
		var options = $.extend({}, options);

		this.each(function() {
				var opt = options.button.split(",");
		...
		function filter(){
			$_Apollo.append('<section class="filter">'+
								'<input type="search" name="filter" id= "filter" placeholder = "Buscar..." autofocus = "on">'+
							'</section>');
		}
		function _delete(){
			$_Apollo.append('<button class="inactive delete" id="button-delete">'+
								'<span class="ico-button ico-user-delete"></span>'+
								'<i>Eliminar</i>'+
							'</button>');
		}
		function add(){
			$_Apollo.append('<button id="button-add">'+
								'<span class="ico-button ico-user-add"></span>'+
								'<i>Nuevo</i>'+
							'</button>');
		}
		function edit(){
			$_Apollo.append('<button class="inactive" id="button-edit">'+
								'<span class="ico-button ico-user-edit"></span>'+
								'<i>Editar</i>'+
							'</button>');
		}
		function block(){
			$_Apollo.append('<button class="inactive" id="button-block">'+
								'<span class="ico-button ico-user-block"></span>'+
								'<i>Bloquear</i>'+
							'</button>');
		}
		function unlock(){
			$_Apollo.append('<button class="inactive" id = "button-unlock">'+
								'<span class="ico-button ico-user-unlook"></span>'+
								'<i>Desbloquear</i>'+
							'</button>');
		}
...
$('input[type=text], input[type=number],input[type=tel], input[type=url],input[type=email], input[type=search]').live({
	click:
	function(){
		$(this).removeClass("validate");
	}
});
$(".button, #link-view, #button-add, #button-edit").live("click",function(){
	$(".loading").show();
})
$("#search-file").live("dblclick",function(){
	caja = $(this);
	preview = "preview"+$(this).attr("data-preview");

	explorerFile(0);
});

$("#insertar-campos").live("click",function(){
	child = childTable() + 1;
	
	var xHML = '<tr id = "'+child+'">'+
					'<td id = "preview'+child+'"></td>'+
					'<td>'+
						'<input type = "text" name = "file[]" id = "search-file" data-preview = "'+child+'">'+
					'</td>'+
					'<td>'+
						'<input type = "text" name = "url[]">'+
					'</td>'+
					'<td>'+
						'<input type = "text" name = "descripFile[]">'+
					'</td>'+
					'<td>'+
						'<input type = "text" name = "creditoFile[]">'+
					'</td>'+
					'<td>'+
						'<center><img src="/Template/img/delete.png" onclick="removeFile('+child+')"></center>'+
					'</td>'+
				'</tr>';
	$("#gallery-files").append(xHML);
})
$("#section_art").live("change",function(){
	$(".loading").show();

	 $.ajax({
		url: "/Component/Ajax/Ajax.php",
		type: 'POST',
		data: ({
			section: $(this).val(),
			action:"cbcategorias"
...
function childTable(){
	var cantidad = 0;
	$("#gallery-files").find('tbody tr').each(function (index) { 
		cantidad = cantidad + 1;
	});
	return cantidad;
}
function removeFile(indx){
	$("#"+indx).remove();
}
...
function explorerFile(container){
	$(".loading").show();
	 ...
	 $.ajax({
		...
		type: 'POST',
		data: ({
			formato:$("#tipo_galeria").val(),
			action:"galeria",
			indx:container
		}),
		success: function(datos){
			$("#datos").modal("show");
			$("#exploradorArchivo").html(datos);
			$(".loading").hide();
		}
	});
}
function imgSelect(imagen,type){

	
    if(caja ==""){
      $("#search-file").val(imagen);

    }else if(caja === "tinymce"){    	
    	if(type == 'Videos'){
    		xHML =''+
    		'<video controls>'+
			'    <source src="'+imagen+'" type="video/mp4">'+
			'    <source src="'+imagen+'"  type="video/ogg">'+
			'    <source src="'+imagen+'"  type="video/WebM">'+
			'    <embed src="'+imagen+'"  type="application/x-shockwave-flash" width="1024" height="798" allowscriptaccess="always" allowfullscreen="true"></embed>'+
			'</video>';
    	}else if(type == 'Imagen'){
    		xHML = '<img src = "'+imagen+'" alt = "" /> ';
    	}else if(type == 'Audio'){
    		xHML = ''+
    		'<audio controls>'+
			'    <source src="'+imagen+'">'+
    		...
    		xHML = '<a href="'+imagen+'" target= "_blank">Click para descargar el archivo</a>';
    	}
    	
    	tinyMCE.execCommand('mceInsertContent',false,'<br>'+xHML)
    }else{
    	var xHML = '<img src = '+imagen+' height = "60px" width = "80px" alt = "" /> '	
		$("#"+preview).html(xHML);
        $(caja).val(imagen);
    }
    $("#datos").modal("hide");
...
function showToolTips(obj,title){
	$(obj).tooltip({
        placement: "bottom",
        title: title,
...
function validateFormFields(frm){	
    ...
    $("#"+frm).find(':input').each(function(){
        ...
        if ($(this).attr('required')) {
        	if($(this).val().trim() == ""){
	        	showToolTips($(this), "Este campo es obligatorio");
	    		send = false;
	    	}
	        switch ($(this).attr('type')){
	        	case 'number':
	        		if(!validateNumber($(this).val())){
	        			showToolTips($(this),"Ingrese solo Números");
	        			send = false;
	        		}
		        	break;
		        case 'tel':
		        	...
		        	break;
		        case 'email':
		        	if(!validateEmil($(this).val())){
		        		showToolTips($(this),"Ingrese un Email Valido");		        		
		        		send = false;
		        	}
		        	break;	
		        case 'url':
		        	if(!validateUrl($(this).val())){
		        		showToolTips($(this),"Ingrese una URL Valida");	
		        		send = false;
		        	}
		        	break;	
	        }
...
function validateUrl(url){
	expr = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
	if ( !expr.test(url) )
        return false;
    else
    	return true;    
}
function MsjUser(msj){
	$(".msj-user").text(msj);
